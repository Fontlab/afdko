    ...
    for (;;) {
        ...
        if (name.length == 0) {
            ...
            if (skipGlyph == 1) {
                ...
                /* on first time through, h->chars has already been          */
                /* initialized to 1, in parseFont I don't change this, as it */
                /* is important for CID fonts, where .notdef is forced into  */
                /* the first slot                                            */
                if (isFirstChar) {
                    isFirstChar = 0;
                    new = &h->chars.array[0];
...
static void saveEncoding(parseCtx h, Encoding *encoding) {
    if (encoding->std) {
        ...
        h->font->synthetic.iEncoding = (i != -1) ? i : encodingAdd(h->g, h->reorder.encoding.cnt, h->reorder.encoding.array, h->reorder.supplement.cnt, h->reorder.supplement.array);
    }
}
...
int parseGetComponent(tcCtx g, int code,
    ...
    chr = (iChar & COMP_CHAR) ? &h->component.chars.array[iChar & ~COMP_CHAR] : &h->chars.array[iChar];
...
static void reorderChars(parseCtx h) {
    ...
    /* We need to include notdef in h->reorder.index, so that in csEndFont, */
    /* procs.GetChar will be called. else the ordering in the final font    */
    /* gets messed up.                                                      */
    {
    ...
    if (h->encoding.std) {
        ...
        i = (h->reorder.supplement.cnt > 0) ? -1 : encodingCheckPredef(custom);
        h->font->iEncoding = (i != -1) ? i : encodingAdd(h->g, h->reorder.encoding.cnt, h->reorder.encoding.array, h->reorder.supplement.cnt, h->reorder.supplement.array);
    }
    h->font->iCharset =
...
static int addStdGlyph(parseCtx h, int id, unsigned gl_id, char **charListptr) {
    ...
    /* but add it anyway if we adding the .notdef,              */
    /* and (g->flags & TC_FORCE_NOTDEF).                        */
    if (h->subset.spec != NULL ||
        h->font->flags & (FONT_CID | FONT_CHAMELEON)) {
        return id; /* Font doesn't match criteria for euro addition */
    }
...
static void addFont(parseCtx h) {
    ...
    if (g->flags & (TC_ADDEURO | TC_FORCE_NOTDEF)) {
        ...
        /* If the font has a zero, we will consider adding a Euro, and the other synthetic glyphs. */
        /* For the Euro, there are additional tests inside addEuro() to see if we will do it.      */
        /* For the other synthetic glyphs, we just check if lowercase x is present.                */
...
void parseFont(tcCtx g, Font *font) {
    ...
    /* If the font is obliqued, then we have already obliqued the */
    /* charstrings, and it is no longer synthetic.                */
...
static void saveMM(parseCtx h, DICT *dict, int iKey) {
    ...
    if (getConvSubr(h, iCDV, &CDV)) {
        ...
        for (i = 0; i < g->nMasters; i++) {
            ...
            for (j = 0; j < nAxes; j++) {
                ...
                /* Each master's position is computed by considering its  */
                /* axis coordinates as a binary number with first axis as */
                /* lsb. This yields a Adobe's master ordering.            */
                order[i] |= n << j;
            }
        }
    ...
    dictSaveNumber(dict, SEEN_KEY(ilenBuildCharArray) ? getKeyInt(h, ilenBuildCharArray, 0) : 32);
...
void parseNew(tcCtx g) {
    ...
    /* readChameleon() and only appears here so that it may be initialized  */
    /* to be inactive.                                                      */
...
static void saveDicts(parseCtx h) {
    ...
    if (!SEEN_KEY(iFontBBox)) {
        /* FontBBox not found; may have been incorrectly specified after the */
        /* CharStrings dictionary in contravention to the Black Book. Since  */
    ...
    /* BuildFont incorrectly specifies the ItalicAngle for MM oblique fonts */
    /* as 0. This code checks for this condition and fixes the fonts known  */
    ...
    if (psMatchValue(h->ps, operator, "currentfile")) {
        ...
        if (strng->type != PS_STRING || integr->type != PS_INTEGER) {
            ...
            if (g->flags & TC_SMALLMEMORY) {
                ...
                /* This trick is based on the assumption that the CIDMap is */
                /* at the beginning of the binary section and that it is    */
                /* followed by the SubrMap and SubrData.                    */
                ...
                /* xxx The CID parser would benefit from using the da for    */
                /* the PostScript part at the beginning of the file and then */
                /* doing the reading locally without using the pstoken       */
                /* library. This would make the code more efficient and more */
                /* readable.                                                 */
...
static void cidReadChars(parseCtx h) {
    ...
    for (icid = 0; icid < h->cid.Count; ++icid) {
        ...
        new->code = 0;
        new->id = CID;
        new->fdIndex = (h->cid.FDBytes == 0) ? 0 : (FDIndex)getOffset(h, pOffset, h->cid.FDBytes);
        new->encrypted = 1;
