...
static Format2 *makeFormat2(cmapCtx h) {
    ...
    for (i = 0; i < fmt->segment.cnt; i++) {
        fmt->segment.array[i].idRangeOffset = (unsigned short)(((fmt->segment.cnt - i - 1) * SEGMENT2_SIZE +
                                                                uint16 +
...
static void setOrdered(Mapping *mapping, int codeBreak, int span, int index) {
    ...
    /* break, then the cost of keeping this as a separate segment is just   */
    /* one new segment break (the following segment break). Else, the cost  */
    /* is adding two new segment breaks - the one to start this segment,    */
    /* and to start the next segment. We mark as NOT ordered those segments */
    /* that are no larger than the threshold.                               */
...
static int partitionRanges(cmapCtx h, Mapping *mapping) {
    ...
    /* and don't merge it with the prior segment or following segment". Now */
    /* we need to set it to really mean 'the glyphID's in this segment are  */
    /* sequential', as this field is used to decide how to use idDelta and  */
    /* idRange.                                                             */
...
static void makeSegment4(hotCtx g, Format4 *fmt, int nSegments, int segment,
    ...
    if (mapping[start].ordered) {
        ...
        fmt->idRangeOffset[segment] = (unsigned short)(uint16 * (nSegments - segment) + uint16 * fmt->glyphId.cnt);
        for (i = start; i <= end; i++) {
            *dnaNEXT(fmt->glyphId) = mapping[i].glyphId;
        }
        /* Future improvement: when you are using idRangeOffset, you do not */
        /* have to set idDelta to 0. This could be used to combine          */
        /* sequential segments that are not ordered.                        */
    }
}
...
static Format4 *makeFormat4(cmapCtx h, unsigned long *length) {
    ...
    while (notOK) {
        ...
        /* hotCalcSearchParams(sizeof(fmt->endCode[0]), nSegments, &fmt->searchRange, &fmt->entrySelector, &fmt->rangeShift); */
