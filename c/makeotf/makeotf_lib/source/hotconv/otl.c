...
#define DeltaBits4 2
#define DeltaBits8 3
    unsigned short *DeltaValue; /* [((EndSize-StartSize+1)* (2^DeltaFormat)+15)/16] */
} DeviceTable;
...
static ClassDefFormat2 *fillClass2(hotCtx g,
    ...
    fmt->ClassRangeRecord = nRanges ? MEM_NEW(g, sizeof(ClassRangeRecord) * nRanges) : NULL;
...
static Offset fillScriptList(hotCtx g, otlTbl t) {
    ...
    for (i = 0; i < nScripts; i++) {
        ...
        script->LangSysRecord = (nLanguages == 0) ? NULL : MEM_NEW(g, sizeof(LangSysRecord) * nLanguages);
